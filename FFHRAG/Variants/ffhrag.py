...
class FFHRAGRetriever:
    ...
    FIG_TOP_K    = 20
    FIG_TOP_N    = 5
    FIG_PROMPT_N = 3        # top-3 captions injected into prompt

    def __init__(
        self,
        loader,
        # ── sweep overrides (None = use class default) ────────
        ...
        # Override instance-level only if explicitly passed
        self.MMR_LAMBDA = mmr_lambda if mmr_lambda is not None else self.MMR_LAMBDA
        self.FIG_BOOST  = fig_boost  if fig_boost  is not None else self.FIG_BOOST
        ...
        self._mmr_pool  = mmr_pool   if mmr_pool   is not None else 50
    ...
    def retrieve(
        ...
        top_chunks = self._mmr(candidate_pool[:self._mmr_pool], k=self.MMR_K, lam=self.MMR_LAMBDA)
        ...
        # ── 10. Select prompt figures — top-3 by CE score ─────
        # Re-sort by CE score alone for prompt injection
        # Higher CE = more query-relevant caption in the prompt
        prompt_figs = sorted(
            selected_figs,
        ...
        return {
            "figures"          : selected_figs,   # all 5 — for eval metrics
            "prompt_figures"   : prompt_figs,      # top-3 — for build_prompt()
            "sections"         : [],
            "chunks"           : top_chunks,
            ...
            "metadata"         : {
                ...
                "n_prompt_figures"  : len(prompt_figs),
